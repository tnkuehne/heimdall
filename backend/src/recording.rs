...
impl RecordingState {
    pub(crate) fn idle(message: impl Into<Option<String>>) -> Self {
        Self {
            recording: false,
            pid: None,
            file: None,
...
pub(crate) fn start() -> Result<RecordingState> {
    ...
    if !process_is_running(pid, Some(&partial_file)) {
        bail!(
...
pub(crate) fn stop() -> Result<RecordingState> {
    ...
    if process_is_running(pid, state.partial_file.as_deref()) {
...
pub(crate) fn status() -> Result<RecordingState> {
    let Some(mut state) = read_state()? else {
        return Ok(RecordingState::idle(None));
    };

    match state.pid {
        Some(pid) if process_is_running(pid, state.partial_file.as_deref()) => {
...
fn process_is_running(pid: i32, partial_file: Option<&Path>) -> bool {
